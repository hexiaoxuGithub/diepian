...
namespace FileOperation
{
    ...
    public class GlobalNewCalip
    {
        ...
        public static GlobalNewCalip LoadObj_GlobalNewCalip()
        {

            GlobalNewCalip actionDoc;
            BinaryFormatter fmt = new BinaryFormatter();
            FileStream fsReader = null;
            try
            {
                fsReader = File.OpenRead("D:\\ParameterProject\\NewCaliperInterface.dat");
                actionDoc = (GlobalNewCalip)fmt.Deserialize(fsReader);
                if (actionDoc == null)
                {
                    actionDoc = new GlobalNewCalip();
                }
                else
                {
                    ;
